...
class MovieParser:
    ...
    def __get_comments(self):
        try:
            info = self.__soup.find_all('div', {'class': 'comment'})
            #info = info.contents

            comments = '['
            for item in info:
                comment_content=item.find('span', {'class': 'short'}).text
                cnt_info=item.find('span',{'class':'comment-info'});
                nick_name=cnt_info.find('a').text
                cnt_time=cnt_info.find('span',{'class':'comment-time'}).text.strip();
                cnt_rate=cnt_info.find('span',{'class':'rating'}).attrs['class'][0].replace('allstar','');
                comments+='{\"name\":\"'+nick_name+'\",\"comment\":\"'+comment_content+'\",\"star\":\"'+cnt_rate+'\",\"time\":\"'+cnt_time+'\"},'
            comments=comments.strip(',')
            comments+=']';
            self.__movie['comments'] = comments.strip(',')
        except:
            pass
    def __get_description(self):
        try:
            info = self.__soup.find('span', {'property': 'v:summary'}).find_parent().find_next_sibling()  
            if not info:
               info = self.__soup.find('span', {'property': 'v:summary'})  
            self.__movie['description'] = info.text.strip()
        except:
            pass
    ...
    def __get_poster(self):
        try:
            info = self.__soup.find('div', {'id': 'mainpic'}).find('img')
            self.__movie['posters'] = info.attrs['src'].strip()
        except:
            pass
